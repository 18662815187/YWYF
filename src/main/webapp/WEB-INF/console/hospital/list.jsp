...
<script type="text/javascript">
	...
	function isShow() {
		...
		//提交 Form表单
		$("#jvForm").attr("action", "<%=basePath%>/hospital/show");
	...
	function optDelete() {
		...
		$("#jvForm").attr("action","<%=basePath%>/hospital/delByIds");
	...
	<div class="box-positon">
		...
		<form class="ropt">
			<input class="add" type="button" value="添加"
				onclick="window.location.href='<%=basePath%>/hospital/toAddOrEdit?type=0'" />
		</form>
		<div class="clear"></div>
	</div>
	<div class="body-box">
		<form action="<%=basePath%>/hospital/list.do" method="post"
			...
			医院名称: <input type="text" name="hosName" value="${hosName}" />审核: <select id="isShow" name="isShow">
		...
		<form id="jvForm">
			<table cellspacing="1" cellpadding="0" width="100%" border="0"
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pagination.list}" var="hos">
						<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'"
							onmouseout="this.bgColor='#ffffff'">
							<td align="center"><input type="checkbox" name="ids"
								value="${hos.id}" /></td>
							<td align="center">${hos.id}</td>
							<td align="center">${hos.hosName}</td>
							<td align="center"><img width="50" height="50"
								src="${hos.pic}"
								onerror="javascript:this.src='<%=request.getContextPath()%>/img/error.png'" /></td>
							<td align="center">${hos.tel}</td>
							<td align="center">${hos.address}</td>
							<td align="center">${hos.money}</td>
							<td align="center"><c:if test="${hos.regType==0}">后台录入</c:if>
							<c:if test="${hos.regType==1}">PC注册</c:if>
							<c:if test="${hos.regType==2}">微网站注册</c:if>
							</td>
							<td align="center"><date:date value="${hos.addtime} " /></td>
							<td align="center"><c:if test="${hos.isShow}">已通过</c:if> <c:if
									test="${!hos.isShow}">未通过</c:if></td>
							<td align="center"><a
								href="<%=basePath%>/hospital/toAddOrEdit?id=${hos.id}" class="pn-opt">修改</a>
								| <a href="<%=basePath%>/hospital/delByIds?ids=${hos.id}"
