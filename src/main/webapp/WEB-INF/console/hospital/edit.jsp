...
<body>
	...
	<h2 class="h2_ch">
		<span id="tabs"> <!-- class的样式中here是选中后未蓝色底，未被选中的是nor白底 --> <a
			...
			<a href="javascript:void(0);" ref="#tab_4" title="商品图片" class="nor">医院认证图片</a>
			<a href="javascript:void(0);" ref="#tab_2" title="商品图片" class="nor">医院图集</a>
			<a href="javascript:void(0);" ref="#tab_3" title="商品描述" class="nor">医院介绍</a>
		</span>
	</h2>
	<div class="body-box" style="float: right">
		<form id="jvForm" action="<%=basePath%>/hospital/apply"
			method="post" enctype="multipart/form-data">
			<table cellspacing="1" cellpadding="2" width="100%" border="0"
				class="pn-ftable">
				<tbody id="tab_1">
					<input type="hidden" name="id" value="${hospital.id}" />
					<input type="hidden" name="CCCID" id="CCCID" value="${hospital.cityId}"></input>
					<input type="hidden" name="AAAID" id="AAAID" value="${hospital.area}"></input>
					<input type="hidden" name="type" value="${type}" />
					<input type="hidden" name="regType" value="${hospital.regType}" />
					<input type="hidden" name="regTime" value="${hospital.addtime}" />
					<input type="hidden" name="qrcode" value="${hospital.qrcode}" />
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							class="pn-frequired">*</span> 医院名称:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="hosName" maxlength="100" size="100"
							value="${hospital.hosName}" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							...
							name="provincesId">
								...
								<c:forEach items="${provinces}" var="province">
									<option value="${ province.provinceid}"
										<c:if test="${hospital.provincesId==province.provinceid}">selected="selected"</c:if>>${ province.province}</option>
								</c:forEach>
					...
					<tr>
						...
						会被替换，此标签内的数据无法进行提交，需要配合隐藏域使用 --> <img width="100" height="100"
							id="allUrl" src="${hospital.pic}" /> <!-- 下方这个隐藏域用于form提交图片数据时使用，value就是用作提交使用的 -->
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="password"
							class="required" name="pwd" value="${hospital.pwd}" maxlength="100"
							size="20" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
					...
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h">GPS坐标X轴:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="locationX" value="${hospital.locationX}"
							maxlength="14" size="20" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h">GPS坐标Y轴:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="locationY" value="${hospital.locationY}"
							maxlength="14" size="20" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="radio"
							...
							<c:if test="${!hospital.isShow}">checked="checked"</c:if> />否&nbsp&nbsp<input
							type="radio" class="required" name="isShow" value="1"
							<c:if test="${hospital.isShow}">checked="checked"</c:if> />是</td>
					</tr>
				...
				<tbody id="tab_4" style="display: none">
					...
					<tr id="tt">
						...
						<td width="80%" class="pn-fcontent"><c:forEach
								items="${hospital.imges}" var="pic">
								<img width="100" height="100" src="${pic}" />
							</c:forEach> <c:if test="${!empty hospital.imges}">
				...
				<tbody id="tab_2" style="display: none">
					...
					<tr id="tt">
						<td width="20%" class="pn-flabel pn-flabel-h"></td>
						<td width="80%" class="pn-fcontent"><c:forEach
								items="${hospital.pics}" var="pic">
								<img width="100" height="100" src="${pic}" />
							</c:forEach> <c:if test="${!empty hospital.pics}">
								<a href="javascript:;" class="pn-opt" onclick="remove()">删除</a>
							</c:if></td>
					</tr>
				</tbody>
				<tbody id="tab_3" style="display: none">
					<tr>
						<td><textarea rows="20" cols="180" id="productdesc"
								name="content">${hospital.content}</textarea></td>
					</tr>
				</tbody>
				<tbody>
					<tr>
