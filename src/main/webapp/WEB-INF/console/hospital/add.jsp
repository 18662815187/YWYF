...
.h2_ch a {
	...
	background: url('<%=basePath%>/images/admin/bg_ch.gif') repeat-x scroll 0% 0%
		transparent;
}
						...
						<td width="80%" class="pn-fcontent"><select name="brandId">
								...
								<c:forEach items="${brands}" var="brand">
									<option value="${brand.id}"
										<c:if test="${brandId == brand.id }">selected="selected"</c:if>>${brand.brandName}</option>
								</c:forEach>
						</select></td>
					</tr>
					<tr>
						...
						<td width="80%" class="pn-fcontent"><select name="brandId">
								...
								<c:forEach items="${factories}" var="brand">
									<option value="${brand.id}"
