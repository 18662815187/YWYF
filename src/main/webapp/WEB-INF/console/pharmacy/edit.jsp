...
<body>
	<div class="box-positon">
		<div class="rpos">当前位置: 医院管理 - 添加</div>
		<form class="ropt">
	...
	<h2 class="h2_ch">
		<span id="tabs"> <!-- class的样式中here是选中后未蓝色底，未被选中的是nor白底 --> <a
			...
			<a href="javascript:void(0);" ref="#tab_4" title="商品图片" class="nor">药店认证图片</a>
			<a href="javascript:void(0);" ref="#tab_2" title="商品图片" class="nor">药店图集</a>
			<a href="javascript:void(0);" ref="#tab_3" title="商品描述" class="nor">药店介绍</a>
		</span>
	</h2>
	<div class="body-box" style="float: right">
		<form id="jvForm" action="<%=basePath%>/pharmacy/saveOrUpdate"
			method="post" enctype="multipart/form-data">
			<table cellspacing="1" cellpadding="2" width="100%" border="0"
				class="pn-ftable">
				<tbody id="tab_1">
					<input type="hidden" name="id" value="${pharmacy.id}" />
					<input type="hidden" name="CCCID" id="CCCID" value="${pharmacy.cityId}"></input>
					<input type="hidden" name="AAAID" id="AAAID" value="${pharmacy.area}"></input>
					<input type="hidden" name="editType" value="${editType}" />
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							class="pn-frequired">*</span> 药店名称:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="pharmacyName" maxlength="100" size="100"
							value="${pharmacy.pharmacyName}" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							...
							name="provincesId">
								...
								<c:forEach items="${provinces}" var="province">
									<option value="${ province.provinceid}"
										<c:if test="${pharmacy.provincesId==province.provinceid}">selected="selected"</c:if>>${ province.province}</option>
								</c:forEach>
					...
					<tr>
						...
						会被替换，此标签内的数据无法进行提交，需要配合隐藏域使用 --> <img width="100" height="100"
							id="allUrl" src="${pharmacy.pic}" /> <!-- 下方这个隐藏域用于form提交图片数据时使用，value就是用作提交使用的 -->
							<input type="hidden" name="pic" id="imgUrl"
							value="${pharmacy.pic}" /> <!-- 此处的name值需要和控制器传入对象的小名一致，MultipartFile pic -->
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="password"
							class="required" name="pwd" value="${pharmacy.pwd}" maxlength="100"
							size="20" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
						...
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="cellPhone" maxlength="100" size="20"
							value="${pharmacy.cellPhone}" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							class="pn-frequired">*</span>店内座机:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="tel" maxlength="100" size="20"
							value="${pharmacy.tel}" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							class="pn-frequired">*</span>负责人:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="manager" maxlength="100" size="20"
							value="${pharmacy.manager}" /></td>
					</tr>
					<tr>
						...
						<td width="80%" class="pn-fcontent"><select name="hosId">
								...
								<c:forEach items="${hospitals}" var="hos">
									<option value="${hos.id}"
										<c:if test="${pharmacy.hosId == hos.id }">selected="selected"</c:if>>${hos.hosName}</option>
								</c:forEach>
						</select></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h">上级药店:</td>
						<td width="80%" class="pn-fcontent"><select name="hosId">
								<option value="">请选择上级(不选择默认为没有上级)</option>
								<c:forEach items="${pharmacies}" var="ph">
									<option value="${ph.id}"
										<c:if test="${pharmacy.parentId == ph.id }">selected="selected"</c:if>>${ph.pharmacyName}</option>
								</c:forEach>
						</select></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="text"
							...
							value="${pharmacy.hosName}" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							class="pn-frequired">*</span> 推荐人ID（查询推荐药店的ID）:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="refereesId" maxlength="100" size="20"
							value="${pharmacy.refereesId}" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h">余额:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="money" maxlength="100" size="10"
							value="${pharmacy.money}" /></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h">结算费率:</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="commission" maxlength="100" size="10"
							value="${pharmacy.commission}" /><span style="color:red">*请输入小数，例如25%则必须输入是0.25</span></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h">结算周期(天):</td>
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="settlement" maxlength="100" size="10"
							value="${pharmacy.settlement}" /><span style="color:red">*整数天数</span></td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							class="pn-frequired">*</span> 药房类型:</td>
						<td width="80%" class="pn-fcontent"><input type="radio"
							class="required" name="type" value="0" checked="checked"
							<c:if test="${pharmacy.type==0}">checked="checked"</c:if> />医院内部药房&nbsp&nbsp<input
							type="radio" class="required" name="type" value="1"
							<c:if test="${pharmacy.type==1}">checked="checked"</c:if> />社会药店</td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
						...
						<td width="80%" class="pn-fcontent"><input type="radio"
							...
							<c:if test="${!pharmacy.isShow}">checked="checked"</c:if> />否&nbsp&nbsp<input
							type="radio" class="required" name="isShow" value="1"
							<c:if test="${pharmacy.isShow}">checked="checked"</c:if> />是</td>
					</tr>
					<tr>
						<td width="20%" class="pn-flabel pn-flabel-h"><span
							class="pn-frequired">*</span> 是否指定为院外药房:</td>
						<td width="80%" class="pn-fcontent"><input type="radio"
							class="required" name="isSpecified" value="0" checked="checked"
							<c:if test="${!pharmacy.isSpecified}">checked="checked"</c:if> />否&nbsp&nbsp<input
							type="radio" class="required" name="isSpecified" value="1"
							<c:if test="${pharmacy.isSpecified}">checked="checked"</c:if> />是</td>
					</tr>
				...
				<tbody id="tab_4" style="display: none">
					...
					<tr id="tt">
						...
						<td width="80%" class="pn-fcontent"><c:forEach
								items="${pharmacy.imges}" var="pic">
								<img width="100" height="100" src="${pic}" />
							</c:forEach> <c:if test="${!empty pharmacy.imges}">
				...
				<tbody id="tab_3" style="display: none">
					<tr>
						<td><textarea rows="20" cols="180" id="productdesc"
								name="content">${pharmacy.content}</textarea></td>
					</tr>
				</tbody>
				<tbody>
					<tr>
