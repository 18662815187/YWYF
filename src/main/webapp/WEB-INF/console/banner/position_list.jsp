...
<script type="text/javascript">
	...
	function toEdit(id) {
		$("#jvForm").attr(
				"action",
				"<%=basePath%>/posstion/toEdit?id=" + id);
	...
	function Delete(id){
		...
		$("#jvForm").attr(
				"action",
				"<%=basePath%>/posstion/deletes?ids=" + id);
		$("#jvForm").attr("method", "post").submit();
	}
	function Deletes(){
		var size = $("input[name=ids]:checked").size();
...
<body>
	...
	<div class="body-box">
		<form action="<%=basePath%>/posstion/list.do" method="post" style="padding-top: 5px;">
		...
		<form id="jvForm">
			<table cellspacing="1" cellpadding="0" border="0" width="100%"
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pagination.list}" var="pos">
						<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'"
							onmouseover="this.bgColor='#eeeeee'">

							<td align="center"><input type="checkbox" value="${pos.id}"
								name="ids" id="${pos.id}" /></td>
							<td align="center">${pos.id}</td>
							<td align="center">${pos.position}</td>
							<td align="center"><a class="pn-opt" href="#"
								onclick="toEdit('${pos.id}');return false;">修改</a>
								| <a class="pn-opt" href="#"
								onclick="Delete('${pos.id}');return false;">删除</a>
