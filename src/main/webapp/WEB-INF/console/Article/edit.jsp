...
<body>
	...
	<div class="body-box" style="float: right">
		<form id="jvForm" action="<%=basePath%>/article/saveOrUpdate"
				...
				<tbody id="tab_1">
					...
					<tr>
						...
						会被替换，此标签内的数据无法进行提交，需要配合隐藏域使用 --> <img width="100" height="100"
							id="allUrl" src="${article.pic}" /> <!-- 下方这个隐藏域用于form提交图片数据时使用，value就是用作提交使用的 -->
							<input type="hidden" name="pic" id="imgUrl"
							value="${article.pic}" /> <!-- 此处的name值需要和控制器传入对象的小名一致，MultipartFile pic -->
					...
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="radio"
							...
							<c:if test="${article.isShow==0}">checked="checked"</c:if> />否&nbsp&nbsp<input
							type="radio" class="required" name="isShow" value="1"
							<c:if test="${article.isShow==1}">checked="checked"</c:if> />是</td>
					</tr>
