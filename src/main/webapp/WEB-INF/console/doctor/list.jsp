...
<script type="text/javascript">
	...
	function isShow() {
		...
		//提交 Form表单
		$("#jvForm").attr("action", "<%=basePath%>/doctor/upByIds");
	...
	function optDelete() {
		...
		$("#jvForm").attr("action","<%=basePath%>/doctor/delByIds");
	...
	<div class="box-positon">
		...
		<form class="ropt">
			<input class="add" type="button" value="添加"
				onclick="window.location.href='<%=basePath%>/doctor/toSaveOrEdit?editType=0'" />
		</form>
		<div class="clear"></div>
	</div>
	<div class="body-box">
		<form action="<%=basePath%>/doctor/list.do" method="post"
			...
			医生名称: <input type="text" name="docName" value="${docName}" />医院名称: <input type="text" name="hosName" value="${hosName}" />审核: <select id="isShow" name="isShow">
		...
		<form id="jvForm">
			<table cellspacing="1" cellpadding="0" width="100%" border="0"
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pagination.list}" var="doc">
						<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'"
							...
							<td align="center">${doc.tel}</td>
							<td align="center">${doc.hosName}</td>
							<td align="center">${doc.authhosname}</td>
							<td align="center">${doc.money}</td>
							<td align="center"><c:if test="${doc.regType==0}">后台录入</c:if>
							<c:if test="${doc.regType==1}">PC注册</c:if>
							<c:if test="${doc.regType==2}">微网站注册</c:if>
							</td>
							<td align="center"><date:date value="${doc.regtime} " /></td>
							<td align="center"><c:if test="${doc.isShow}">已通过</c:if> <c:if
									test="${!doc.isShow}">未通过</c:if></td>
							<td align="center"><a
								href="<%=basePath%>/doctor/toSaveOrEdit?id=${doc.id}" class="pn-opt">修改</a>
								| <a href="<%=basePath%>/doctor/delByIds?ids=${doc.id}"
