...
<body>
	...
	<div class="body-box" style="float: right">
		<form id="jvForm" action="<%=basePath%>/DisType/apply" method="post"
			enctype="multipart/form-data">
			<input type="hidden" value="${disease.id}" name="id" />
			<input type="hidden" value="${disease.addTime}" name="addTime" />
				...
				<tbody>
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="text"
							class="required" name="name" value="${disease.name}"
							maxlength="100" /></td>
					</tr>
					<tr>
						...
						<td width="80%" class="pn-fcontent"><select
							name="funcId">
								...
								<c:forEach items="${funTypes}" var="t">
									<option value="${t.id}"
										<c:if test="${t.id == disease.funcId}">selected="selected"</c:if>>${t.name}</option>
								</c:forEach>
						</select></td>
					</tr>
				</tbody>
