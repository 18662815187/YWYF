...
<script type="text/javascript">
	...
	function toEdit(id) {
		$("#jvForm").attr(
				"action",
				"<%=basePath%>/funType/toAddOrEdit?id=" + id);
	...
	function Delete(id){
		...
		$("#jvForm").attr(
				"action",
				"<%=basePath%>/funType/delById?id="+id);
		$("#jvForm").attr("method", "post").submit();
	}
	function Deletes(){
		var size = $("input[name=ids]:checked").size();
...
<body>
	...
	<div class="body-box">
		<form action="<%=basePath%>/funType/list.do" method="post"
			style="padding-top: 5px;">
			分类名称: <input type="text" name="FtName" value="${FtName}" /> 所属分类：<select
				name="par_id">
				<option value="">请选择(默认全部)</option>
				<c:forEach items="${proTypes}" var="pt">
					<option value="${pt.id}"
						<c:if test="${par_id==pt.id}">selected="selected"</c:if>>${pt.typeName}</option>
				</c:forEach>
		...
		<form id="jvForm">
			<table cellspacing="1" cellpadding="0" border="0" width="100%"
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pagination.list}" var="funType">
						<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'"
							onmouseover="this.bgColor='#eeeeee'">

							<td align="center"><input type="checkbox"
								value="${funType.id}" name="ids" id="${funType.id}" /></td>
							<td align="center">${funType.id}</td>
							<td align="center">${funType.name}</td>
							<td align="center">${funType.parName}</td>
							<td align="center"><a class="pn-opt" href="#"
								onclick="toEdit('${funType.id}');return false;">修改</a> | <a
								class="pn-opt" href="#"
								onclick="Delete('${funType.id}');return false;">删除</a> <%-- 								<a class="pn-opt" onclick="if(!confirm('您确定删除吗？')) {return false;}" href="delete.do?id=${data.id}&name=${optionName}&isDisplay=${isshow}&pageNo=${pagination.pageNo}">删除</a> --%>
		...
		<div class="page pb15">
			<span class="r inb_a page_b"><span
				...
				&nbsp&nbsp&nbsp <c:forEach items="${pagination.pageView}" var="page">
					${page}
			</c:forEach> </span>
		</div>
