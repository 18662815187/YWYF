...
<script type="text/javascript">
	...
	function toEdit(id) {
		$("#jvForm").attr(
				"action",
				"<%=basePath%>/DisType/toAddOrEdit?id=" + id);
	...
	function Delete(id){
		...
		$("#jvForm").attr(
				"action",
				"<%=basePath%>/DisType/delById?id=" + id);
		$("#jvForm").attr("method", "post").submit();
	}
	function Deletes(){
		var size = $("input[name=ids]:checked").size();
...
<body>
	...
	<div class="body-box">
		<form action="<%=basePath%>/DisType/list.do" method="post"
			style="padding-top: 5px;">
			分类名称: <input type="text" name="dis_name" value="${dis_name}" />
			二级分类: <select id="funTypeId" name="funTypeId">
				<option value="">请选择(默认全部)</option>
				<c:forEach items="${funTypes}" var="fun">
					<option value="${fun.id}" <c:if test="${funTypeId==fun.id}">selected="selected"</c:if>>${fun.name}</option>
				</c:forEach>
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pagination.list}" var="type">
						<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'"
							...
							<td align="center">${type.par_name}</td>
							<td align="center"><a class="pn-opt" href="#"
								onclick="toEdit('${type.id}');return false;">修改</a> | <a
								class="pn-opt" href="#"
								onclick="Delete('${type.id}');return false;">删除</a> <%-- 								<a class="pn-opt" onclick="if(!confirm('您确定删除吗？')) {return false;}" href="delete.do?id=${data.id}&name=${optionName}&isDisplay=${isshow}&pageNo=${pagination.pageNo}">删除</a> --%>
