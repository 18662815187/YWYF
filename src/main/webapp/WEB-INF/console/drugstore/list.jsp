...
<script type="text/javascript">
	...
	function isShow() {
		...
		//提交 Form表单
		$("#jvForm").attr("action", "<%=basePath%>/drugstore/show");
	...
	<div class="box-positon">
		...
		<form class="ropt">
			<input class="add" type="button" value="添加"
				onclick="javascript:window.location.href='<%=basePath%>/drugstore/init'" />
		</form>
		<div class="clear"></div>
	</div>
	<div class="body-box">
		<form action="<%=basePath%>/brand/list.do" method="post" style="padding-top: 5px;">
			药店名称: <input type="text" name="drugstore_name" value="${drugstore_name}" />
			 <select name="isShow">
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pagination.list}" var="brand">
						<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'"
							...
							<td align="center">${brand.pharmacy}</td>
							<td align="center"><c:if test="${brand.status}">是</c:if> <c:if
									test="${!brand.status}">否</c:if></td>
